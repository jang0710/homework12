...
import com.example.applemarket.Constants.ITEM_INDEX
import com.example.applemarket.Constants.ITEM_OBJECT
import com.example.applemarket.databinding.ActivityMainBinding

class MainActivity : AppCompatActivity() {

    lateinit var activityReultLauncher: ActivityResultLauncher<Intent>
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        adapter.itemClick = object : AppleAdapter.ItemClick {
            override fun onClick(view: View, position: Int) {
                ...
                activityReultLauncher.launch(intent)
            }
        }
        ...
        activityReultLauncher = registerForActivityResult(ActivityResultContracts.StartActivityForResult()) {
            if (it.resultCode == RESULT_OK) {
                val itemIndex = it.data?.getIntExtra("itemIndex", 0) as Int
                val isLike = it.data?.getBooleanExtra("isLike", false) as Boolean

                if(isLike) {
                    dataList[itemIndex].aLike = true
                    dataList[itemIndex].InterestCnt += 1
                }else {
                    if(dataList[itemIndex].aLike) {
                        dataList[itemIndex].aLike = false
                        dataList[itemIndex].InterestCnt -= 1
                    }
                }
